...
function s:initialise()
    ...
    if !exists( 'g:timestamp_UseSystemCalls' )
        ...
        if exists('g:timestamp_username')
            ...
            let s:username = s:getValue( v:shell_error ? 'unknown' : s:username, 'NONE', $USER, $LOGNAME, $USERNAME)
        endif
...
function s:setup_autocommand()
    if has('autocmd')
        if ! exists( 's:autocomm' )
            ...
            " use BufWritePre cause issue that every time when undo
            " the cursor jump to the head, annoying, but BufUnload doesn't work???
            let s:autocomm = "autocmd BufWritePre " . l:automask . " :call s:timestamp()"
        endif
